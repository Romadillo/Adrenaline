...
public class BeheadingProcedureProcedure {
		...
		if (((EnchantmentHelper.getEnchantmentLevel(BeheadingEnchantment.enchantment,
			...
			if (((entity instanceof ZombieEntity) && (Math.random() < 0.5))) {
				if (world instanceof World && !world.isRemote()) {
					ItemEntity entityToSpawn = new ItemEntity((World) world, x, y, z, new ItemStack(Blocks.ZOMBIE_HEAD));
				...
				if (world instanceof World && !world.isRemote()) {
					ItemEntity entityToSpawn = new ItemEntity((World) world, x, y, z, new ItemStack(Blocks.WITHER_SKELETON_SKULL));
					entityToSpawn.setPickupDelay((int) 10);
					world.addEntity(entityToSpawn);
				}
			}
